<?php
// Sample kost data
$kosts = [
  1 => [
    "id" => 1,
    "name" => "Kost Gina Tipe Standard",
    "price" => 700000,
    "location" => "Bandung",
    "image" => "depan.jpeg",
    "all_images" => ["depan.jpeg", "dalem.jpeg", "samping.jpeg"],
    "spek" => ["3x4 meter", "Termasuk listrik"],
    "rules" => [
            "Tipe ini bisa diisi maks. 2 orang/ kamar",
            "Boleh pasutri",
            "Wajib sertakan surat nikah saat pengajuan sewa",
            "Tidak boleh bawa anak",
            "Deposit: Dikembalikan di akhir periode sewa jika tidak ditemukan kerusakan pada kamar.",
            "Denda keterlambatan: Dibebankan jika penyewa terlambat bayar tagihan sewa."
    ],
    "fasilitas" => [
      "Kasur",
      "Lemari Baju",
      "Ventilasi",
      "Jendela",
      "Fasilitas kamar mandi"
    ],
    "lokasi" => [
      "Soekarno Hatta (By Pass)",
      "MIM (Metro Indah Mall)",
      "Jalan Rancabolang Raya",
      "Jalan Ciwastra",
      "Lokasi dekat terminal Margahayu, minimarket, apotek, Rumah Sakit dll"
    ],
    "biaya_lain" => [
      "Range harga kamar 700.000-1.400.000/bulan",
      "Deposit 50% dari harga sewa per bulan",
      "Pesan tempat minimal 30% dari harga sewa per bulan",
      "Mobil 100.000",
      "Tambah orang 150.000",
      "Kelebihan daya pemakaian listrik 50.000"
    ],
    "payment" => [
      "Bisa bayar DP (uang muka) dulu",
      "Biaya DP adalah 30% dari biaya yang dipilih"
    ],    
    "description" => "Keterangan biaya lainnya Range harga kamar 700.000 - 1.000.000/bulan. Deposit 50% dari harga sewa per bulan. Pesan tempat minimal 30% dari harga sewa per bulan. Mobil 100.000; Tambahan orang 150.000; Kelebihan daya pemakaian listrik 50.000."
  ],
  2 => [
    "id" => 2,
    "name" => "Kost Alpha Tipe C",
    "price" => 700000,
    "location" => "Bandung",
    "image" => "kosan2depan.jpg",
    "all_images" => ["kosan2depan.jpg", "kosan2dalem.jpg", "kosan2kamar.jpg"],
    "spek" => ["3x4 meter", "Termasuk listrik"],
    "rules" => [
            "Tipe ini bisa diisi maks. 2 orang/ kamar",
            "Boleh pasutri",
            "Wajib sertakan surat nikah saat pengajuan sewa",
            "Tidak boleh bawa anak",
            "Deposit: Dikembalikan di akhir periode sewa jika tidak ditemukan kerusakan pada kamar.",
            "Denda keterlambatan: Dibebankan jika penyewa terlambat bayar tagihan sewa."
    ],
    "fasilitas" => [
      "Kasur",
      "Lemari Baju",
      "Ventilasi",
      "Jendela",
      "Fasilitas kamar mandi"
    ],
    "lokasi" => [
      "Soekarno Hatta (By Pass)",
      "MIM (Metro Indah Mall)",
      "Jalan Rancabolang Raya",
      "Jalan Ciwastra",
      "Lokasi dekat terminal Margahayu, minimarket, apotek, Rumah Sakit dll"
    ],
    "biaya_lain" => [
      "Range harga kamar 700.000-1.400.000/bulan",
      "Deposit 50% dari harga sewa per bulan",
      "Pesan tempat minimal 30% dari harga sewa per bulan",
      "Mobil 100.000",
      "Tambah orang 150.000",
      "Kelebihan daya pemakaian listrik 50.000"
    ],
    "payment" => [
      "Bisa bayar DP (uang muka) dulu",
      "Biaya DP adalah 30% dari biaya yang dipilih"
    ],
    "description" => "Tipe ini bisa diisi maks. 1 orang/kamar Tidak untuk pasutri Tidak boleh bawa anak Deposit Dikembalikan di akhir periode sewa jika tidak ditemukan kerusakan pada kamar."
  ],
  3 => [
    "id" => 3,
    "name" => "Kost Graha Hermawan",
    "price" => 700000,
    "location" => "Bandung",
    "image" => "kosan3depan.jpg",
    "all_images" => ["kosan3depan.jpg", "kosan3dalem.jpg", "kosan3kamar.jpg"],
    "spek" => ["3x4 meter", "Termasuk listrik"],
    "rules" => [
            "Tipe ini bisa diisi maks. 2 orang/ kamar",
            "Boleh pasutri",
            "Wajib sertakan surat nikah saat pengajuan sewa",
            "Tidak boleh bawa anak",
            "Deposit: Dikembalikan di akhir periode sewa jika tidak ditemukan kerusakan pada kamar.",
            "Denda keterlambatan: Dibebankan jika penyewa terlambat bayar tagihan sewa."
    ],
    "fasilitas" => [
      "Kasur",
      "Lemari Baju",
      "Ventilasi",
      "Jendela",
      "Fasilitas kamar mandi"
    ],
    "lokasi" => [
      "Soekarno Hatta (By Pass)",
      "MIM (Metro Indah Mall)",
      "Jalan Rancabolang Raya",
      "Jalan Ciwastra",
      "Lokasi dekat terminal Margahayu, minimarket, apotek, Rumah Sakit dll"
    ],
    "biaya_lain" => [
      "Range harga kamar 700.000-1.400.000/bulan",
      "Deposit 50% dari harga sewa per bulan",
      "Pesan tempat minimal 30% dari harga sewa per bulan",
      "Mobil 100.000",
      "Tambah orang 150.000",
      "Kelebihan daya pemakaian listrik 50.000"
    ],
    "payment" => [
      "Bisa bayar DP (uang muka) dulu",
      "Biaya DP adalah 30% dari biaya yang dipilih"
    ],
    "description" => "Listrik akan ditagihkan setiap tanggal 1, biaya penggunaan Rp. 2.500/Kwh. Perhitungan berdasarkan penggunaan setiap bulannya. Keterangan biaya lainnya Listrik akan ditagih dari penggunaan perbulan. Biaya listrik: Rp. 2.500,00/Kwh"
  ]
];
?>
